...
- The CLI must **always explicitly request `scope=admin-api`**: with scope omitted, the per-client policy filters the global default (`mcp`) down to an empty scope set — works today only because scopes aren't enforced yet.
