...
✓ Wrote flagsmith.json — try `flagsmith flag list`
```
...
```
$ cd apps/api/handlers && flagsmith flag list
...
- Prompts and the browser login require a TTY. Without one (CI, pipes, redirected stdin) — or with `--yes` set — `flagsmith init` never prompts and never opens a browser.
- `--yes` and `--no-input` are aliases of one flag, also settable as `FLAGSMITH_NO_INPUT=1`: never prompt, never open a browser, answer confirmations (like overwriting an existing `flagsmith.json`) affirmatively. It exists to get CI-identical behaviour in a terminal, and it is deliberately the one input with no `flagsmith.json` field — "always overwrite" is invocation behaviour, not repo state anyone should commit.
...
- `--environment` accepts a client-side key always; a name only when credentials can resolve it (ambiguity is an error).
